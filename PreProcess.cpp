...
vector<string> PreProcess::getNextLine()
{
	...
	for (int k = 0; k < str_buffer.size(); k++) {
		...
		//处理字符串,规定标准字符串格式 "+空格+内容+空格+" , 有转义符或者前面有'且不在读取字符串时，不应该认定为是字符串的起始标志
			...
			if ((k == 0 || (str_buffer[k - 1] != '\\') && !(str_buffer[k - 1] == '\'' && !is_string)) && !(is_single_comment || is_multi_comment)) {
		...
		// 处理单行注释
		else if ((str_buffer[k] == '/') && (k + 1 < str_buffer.size()) && (str_buffer[k + 1] == '/')&& !(is_string || is_char) &&!(is_include && is_left_trail)) {
		...
		//处理+,-,*,/,%，=,<,>, +=,-=,*=,/= ，%=,==,<=.>=
			...
			|| str_buffer[k] == '/' || str_buffer[k] == '%' || str_buffer[k] == '='
			|| str_buffer[k] == '<' || str_buffer[k] == '>')&&!(is_string||is_char) && !(str_buffer[k]!='>' && is_include && is_left_trail)) 
		{
		...
		//处理 !,|,&,~，^
			...
			|| str_buffer[k] == '|' || str_buffer[k] == '&')&&!(is_string || is_char) && !(is_include && is_left_trail)) {
			addSpaceToSign(str_buffer, k, 1);
			k += 2;
		}

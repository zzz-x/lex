...
void PreProcess::dealDefineSentence(const string& str)
{
	...
	vector<string> second_key_list;
	while (!str_in.eof()) {
		string tmp;
		str_in >> tmp;
		if (tmp != "")
			second_key_list.push_back(tmp);
		//读到 “就转化为getc去读
		if (tmp == "\"") {
			string str_to_read;
			int prev = 0, cur = 0;
			...
			str_to_read.pop_back();	//去除多读的空格
			second_key_list.push_back(str_to_read);
			second_key_list.push_back("\"");	//加入已经被读掉的"
		}
	}
...
vector<string> PreProcess::getNextLine()
{
	...
	for (int k = 0; k < str_buffer.size(); k++) {
		...
		if (str_buffer[k] == ' ')
			continue;
		//处理字符串,规定标准字符串格式 "+空格+内容+空格+"
		else if (str_buffer[k] == '"'&&((k-1>=0&&str_buffer[k-1]!='\\'&&str_buffer[k-1]!='\'') || k == 0) 
			&& !(is_single_comment || is_multi_comment||(is_include&&is_left_trail))) {
			addSpaceToSign(str_buffer, k, 1);
			k += 2;
			is_string = !is_string;
		}
		// 处理单行注释
		else if ((str_buffer[k] == '/') && (k + 1 < str_buffer.size()) && (str_buffer[k + 1] == '/')&&!is_string&&!(is_include && is_left_trail)) {
		...
		// 处理 #include和 #define
		else if (str_buffer[k] == '#'&&!is_string) {
		...
		//处理 << ,>>,++,--,&&,||,::
			...
			|| str_buffer[k] == ':')&&!is_string && !(is_include && is_left_trail)
			&& (k + 1 < str_buffer.size())
		...
		//处理 ,{.}[,],(,),;,:
			...
			|| str_buffer[k] == ')' || str_buffer[k] == ']' || str_buffer[k] == '{'
			 || str_buffer[k] == ';' || str_buffer[k] == ','||str_buffer[k] == ':')&& !is_string)
		{
		...
		//处理+,-,*,/,%，=,<,>, +=,-=,*=,/= ，%=,==,<=.>=
			...
			|| str_buffer[k] == '/' || str_buffer[k] == '%' || str_buffer[k] == '='
			|| str_buffer[k] == '<' || str_buffer[k] == '>')&&!is_string && !(str_buffer[k]!='>' && is_include && is_left_trail)) {
		...
		//处理 !,|,&,~，^
			...
			|| str_buffer[k] == '|' || str_buffer[k] == '&')&&!is_string && !(is_include && is_left_trail)) {
			addSpaceToSign(str_buffer, k, 1);
			k += 2;
		}
	}
